...
if File.exists?('/tmp/mk-update/bin/lldptool')
  unless `pgrep lldpad` != ""
    if File.exists?('/var/lib/lldpad')
#      Process.wait
#      pid = Process.spawn("/tmp/mk-update/bin/lldpad -d")
#      Process.detach(pid)
#      Process.wait
    else
      fork { exec "mkdir /var/lib/lldpad" }
    end
    fork { exec("for i in `cat /proc/net/dev|cut -d : -f 1 |grep en`; do /tmp/mk-update/bin/lldptool set-lldp -i $i adminStatus=rxtx; done") }
    fork { exec("/tmp/mk-update/bin/lldpad -d") }
    sleep 30
    Process.wait
  end
  lldp = {
  ...
  Facter.value('interfaces').split(/,/).grep_v(/^lo$|^bond[0-9]/).each do |interface|
    ...
    lldp.each_pair do |key, value|
      Facter.add("lldp_neighbor_#{key}_#{interface}") do
        setcode do
          result = ""
#          output = Facter::Util::Resolution.exec("/tmp/mk-update/bin/lldptool get-tlv -i #{interface} -V #{value} 2>/dev/null")
          output = Facter::Util::Resolution.exec("/tmp/mk-update/bin/lldptool get-tlv -n -i #{interface} -V #{value} 2>/dev/null")
          if not output.nil?
            case key
            when 'sysName', 'MTU'
              output.split("\n").each do |line|
